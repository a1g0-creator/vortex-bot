...
class BitgetAdapter(BaseExchange):
    ...
    def _pt(self) -> str:
        """Возвращает v2-совместимый тип рынка фьючерсов по умолчанию."""
        return "USDT-FUTURES"
    ...
    async def _ensure_http_session(self) -> None:
        ...
        ВАЖНО: НЕ добавляем paptrading в default headers, чтобы public ручки не ловили 404.
        """
    ...
    def _get_headers(self, method: str, endpoint: str, params: dict = None) -> Dict[str, str]:
        """
        Базовые заголовки для REST V2.
        В demo добавляем 'paptrading: 1' только для приватных ручек (через _is_private_endpoint).
        Подпись приватных запросов добавляется отдельно (_generate_auth_headers).
        """
        ...
        if self.testnet and self._is_private_endpoint(endpoint):
            headers["paptrading"] = "1"  # строка "1", нижний регистр ключа
        return headers
    ...
    async def _make_request(
        ...
        try:
            ...
            if signed:
                ...
                if self.testnet and "paptrading" not in headers:
            ...
            async with self.session.request(
                ...
                if resp.status >= 400:
                    self.logger.error("BitgetAdapter: HTTP %s %s failed (%s): %s",
                                      method, endpoint, resp.status, text)
                    return None
    ...
    def _extract_data_from_response(self, response: Any) -> Any:
        ...
        Обрабатывает разные форматы возврата _make_request.
    
        Args:
            response: Ответ от _make_request
        ...
        if isinstance(response, dict):
            # Если есть поле data, извлекаем его
            if "data" in response:
                return response.get("data")
            # Иначе возвращаем сам response (уже развернутые данные)
            return response
    
        # Если response уже список или другой тип данных
        return response
    ...
    async def get_balance(self, coin: str = "USDT") -> Optional[Balance]:
        """Баланс по маржинальной монете (V2, demo/prod). Корректно обрабатывает кейсы демо, где equity=0, а available>0."""
        try:
            # Основной путь: single account (обязательны symbol+productType+marginCoin)
            resp = await self._make_request(
                "GET",
                "/api/v2/mix/account/account",
                params={
                    "symbol": "btcusdt",
                    "productType": self._pt(),       # "USDT-FUTURES"
                    "marginCoin": coin.lower(),      # "usdt"
                },
                signed=True,
            )
            ...
            # Извлекаем все релевантные поля
            equity_raw = float(node.get("equity", 0) or 0)
            usdt_equity = float(node.get("usdtEquity", 0) or 0)
            ...
            # Правило вычисления "кошелька"
            if equity_raw > 0:
                wallet = equity_raw
            elif usdt_equity > 0:
                wallet = usdt_equity
            else:
                # На демке equity часто 0; используем доступные средства + замороженные + PnL
                wallet = max(0.0, available + max(frozen, 0.0) + float(unrealized))
    ...
    async def get_wallet_balance(self) -> List[Balance]:
        """Список фьючерсных балансов (V2). Для демки Bitget корректно считает 'wallet' при equity=0."""
        try:
            balances: List[Balance] = []

            # Основной путь: accounts(list) по productType
            resp = await self._make_request(
                "GET",
                "/api/v2/mix/account/accounts",
                params={"productType": self._pt()},   # "USDT-FUTURES"
                signed=True,
            )
            ...
            for acc in items or []:
                ...
                equity_raw = float(acc.get("equity", 0) or 0)
                usdt_equity = float(acc.get("usdtEquity", 0) or 0)
            ...
            # Fallback: single account (если list пустой/не дал данных)
    ...
    async def get_ticker(self, symbol: str) -> Optional[Ticker]:
        """
        Получить тикер фьючерсного символа Bitget (v2).
        Требует обязательный productType. Возвращает Ticker со всеми полями,
        которые ожидает наш dataclass (turnover_24h, price_change_24h, high_24h, low_24h и пр.).
        """
        try:
            # Для REST-маркет ручек Bitget v2 используем «торговый» символ без S-префикса
            sym = self._normalize_symbol_for_trading(symbol)
            pt = self._pt()

            # 1) Основной вызов — одиночный тикер
            ...
            # Безопасное извлечение данных с учетом разных форматов ответа
            item = None
            if resp is not None:
                if isinstance(resp, dict):
                    # resp может быть {"code": "...", "data": {...}}
                    # или может быть уже непосредственно data
                    if "data" in resp:
                        data = resp.get("data")
                        ...
                        # resp уже является data объектом
                        item = resp
                elif isinstance(resp, list) and resp:
                    # resp уже является списком данных
                    item = resp[0]

            # 2) Фолбэк — список всех тикеров по productType
            if not item:
                self.logger.debug(f"get_ticker: fallback to tickers list for {sym}")
                resp2 = await self._make_request(
                    "GET",
                ...
                # Безопасное извлечение данных из resp2
                data2 = None
                if resp2 is not None:
                    if isinstance(resp2, dict):
                        if "data" in resp2:
                            ...
                            # resp2 может быть объектом с тикерами на верхнем уровне
                            data2 = resp2
                    elif isinstance(resp2, list):
                        # resp2 уже является списком
                        data2 = resp2
                    else:
                        data2 = []
            
                # Поиск нужного символа в списке
                if data2:
                    if isinstance(data2, list):
                        item = next((row for row in data2 if str(row.get("symbol")) == sym), None)
                    elif isinstance(data2, dict):
                        # Возможно единственный элемент вернулся как dict
                        if data2.get("symbol") == sym:
                            item = data2

            if not item:
                self.logger.error(f"get_ticker: no ticker data for {sym} (productType={pt})")
                return None

            # Маппинг из Bitget v2:
            # lastPr / bidPr / askPr / high24h / low24h / change24h / baseVolume / quoteVolume / usdtVolume / ts
            ...
            last_pr = safe_float(item.get("lastPr", 0))
            bid_pr = safe_float(item.get("bidPr", 0))
            ask_pr = safe_float(item.get("askPr", 0))
            high24 = safe_float(item.get("high24h", 0))
            low24 = safe_float(item.get("low24h", 0))
            change24 = safe_float(item.get("change24h", 0))
            base_vol = safe_float(item.get("baseVolume", 0))
            quote_vol = safe_float(item.get("quoteVolume", 0))
            usdt_vol = safe_float(item.get("usdtVolume", 0))

            # Для USDT-перпов в качестве turnover_24h логичнее брать usdtVolume; если пусто — quoteVolume
            turnover_24h = usdt_vol if usdt_vol > 0 else quote_vol

            # Процент изменения за 24ч: в v2 'change24h' — абсолютное изменение
            # Переведём в % относительно цены сутки назад (last - change)
            price_change_pct = 0.0
            if last_pr > 0:
                denom = (last_pr - change24)
                if denom != 0:
                    price_change_pct = (change24 / abs(denom)) * 100.0

            # Собираем Ticker БЕЗ timestamp (его нет в dataclass)
            ticker = Ticker(
                symbol=sym,
                last_price=last_pr,
                bid_price=bid_pr,
            ...
            # Опционально: добавляем алиасы для совместимости с тестером (как в BybitAdapter)
            ...
            self.logger.error(f"get_ticker error: {e}", exc_info=True)
            return None
    ...
    async def get_server_time(self) -> int:
        """
        Возвращает время сервера Bitget (мс).
        1) /api/v2/public/time — публичная ручка без paptrading (твоя проверка curl уже подтвердила её работоспособность).
        2) fallback: /api/spot/v1/public/time.
        """
        try:
            v2 = await self._make_request("GET", "/api/v2/public/time", params={})
            if isinstance(v2, dict) and "serverTime" in v2:
                return int(v2["serverTime"])

            v1 = await self._make_request("GET", "/api/spot/v1/public/time", params={})
            if isinstance(v1, (int, float, str)):
                return int(v1)
            if isinstance(v1, dict) and "serverTime" in v1:
                return int(v1["serverTime"])

            self.logger.error("get_server_time: both v2 & v1 attempts failed (no serverTime)")
            return 0
        except Exception as e:
            self.logger.error(f"get_server_time error: {e}", exc_info=True)
            return 0
