"""
Основной торговый движок с интеграцией риск-менеджмента
Перенос логики из SimpleTradingBot с модульной архитектурой + система управления рисками
"""
...
from core.indicators import VortexBandsAnalyzer, technical_indicators
from core.risk_manager import RiskManager, get_risk_manager, set_risk_manager
...
class TradingEngine:
    ...
    async def initialize(self) -> bool:
        """
        Инициализация торгового движка с риск-менеджментом
        ...
        try:
            ...
            # НОВОЕ: Инициализация риск-менеджера
            if not await self.risk_manager.initialize():
                self.logger.error("Не удалось инициализировать риск-менеджер")
                return False
    ...
    async def _open_position(self, symbol: str, signal: Dict[str, Any]) -> bool:
        """
        Открытие позиции с проверкой риск-лимитов (PRE-TRADE CHECK)
        """
        try:
            ...
            # НОВОЕ: PRE-TRADE RISK CHECK
            permission, risk_reason = await self.risk_manager.check_trade_permission(
                symbol=symbol,
                side=signal["signal"],
                position_value=position_value,
                leverage=1  # Получаем из настроек или конфига
            )
            
            if not permission:
                self.logger.warning(f"🛑 {symbol} - Trade blocked by risk manager: {risk_reason}")
                return False
            ...
            if order_info:
                ...
                # НОВОЕ: POST-TRADE UPDATE - обновляем метрики риск-менеджера
                await self._update_risk_metrics_after_trade(
                    symbol=symbol,
                    side=signal["signal"],
                    quantity=float(quantity_str),
                    price=current_price,
                    pnl=0.0  # При открытии PnL = 0
                )
                ...
                self.logger.info(
                    f"✅ {symbol} - Позиция открыта: {signal['signal']} {quantity_str} @ {current_price:.6f}"
                )
                
                return True
            else:
                self.logger.error(f"{symbol} - Не удалось разместить ордер")
                return False
    ...
    async def _close_position_for_reversal(self, symbol: str):
        """
        Закрытие позиции для реверса с обновлением риск-метрик
        """
        try:
            ...
            ticker = await self.exchange.get_ticker(symbol)
            if not ticker:
                self.logger.error(f"{symbol} - Не удалось получить тикер для закрытия")
                return
            ...
            if order_info:
                # НОВОЕ: POST-TRADE UPDATE - обновляем метрики с реализованным P&L
                await self._update_risk_metrics_after_trade(
                    symbol=symbol,
                    side="Close",
                    quantity=float(size_str),
                ...
                self.logger.info(f"{symbol} - Позиция закрыта для реверса, P&L: {pnl:+.2f} USDT")
    ...
    async def _update_single_position(self, symbol: str, current_price: float):
        """
        Управление позицией с экстренным стоп-лоссом и обновлением риск-метрик
        НЕТ автоматических SL/TP - только реверс по сигналам + экстренная защита
        """
        try:
            if symbol not in self.positions:
                return
            ...
            if profit_pct <= -20.0:
                self.logger.error(
                    f"🚨 {symbol} - ЭКСТРЕННЫЙ СТОП-ЛОСС: {profit_pct:.1f}%"
                )
                ...
                if size_str != "0":
                    ...
                    if order_info:
                        # НОВОЕ: POST-TRADE UPDATE при экстренном закрытии
                        await self._update_risk_metrics_after_trade(
                            symbol=symbol,
                            side="Emergency_Close",
                            quantity=float(size_str),
                            price=current_price,
                            pnl=unrealized_pnl
                        )
                        ...
                        # Отправляем уведомление об экстренном закрытии
                        await self._send_close_notification(symbol, f"EMERGENCY Stop Loss: {profit_pct:.2f}%", profit_pct)
                        
                        self.logger.info(
                            f"{symbol} - Позиция закрыта экстренно: P&L {unrealized_pnl:+.2f} USDT"
                        )
            else:
                # Обычный мониторинг позиции - логируем состояние каждые 10% изменения
    ...
    async def _update_risk_metrics_after_trade(self, symbol: str, side: str, 
                                             quantity: float, price: float, pnl: float):
        """
        Обновление метрик риск-менеджера после сделки (POST-TRADE UPDATE)
        ...
        """
        try:
            await self.risk_manager.update_after_trade(
                symbol=symbol,
                side=side,
                quantity=quantity,
                price=price,
            ...
            self.logger.error(f"Ошибка обновления риск-метрик: {e}")
    ...
    async def _close_position(self, symbol: str, reason: str):
        ...
        try:
            ...
            if order_info:
                # НОВОЕ: POST-TRADE UPDATE при закрытии
                await self._update_risk_metrics_after_trade(
                    symbol=symbol,
                    side="Close",
                    quantity=float(size_str),
    ...
    # НОВЫЕ методы для интеграции с Telegram и риск-менеджментом
    ...
    async def set_risk_parameter(self, path: str, value: Any) -> Tuple[bool, str]:
        ...
        try:
            if self.risk_manager:
                return await self.risk_manager.set_risk_parameter(path, value)
            else:
    ...
    async def reset_daily_risk_counters(self) -> bool:
        ...
        try:
            if self.risk_manager:
                return await self.risk_manager.reset_daily_counters()
            else:
                return False
        except Exception as e:
    ...
    async def reset_weekly_risk_counters(self) -> bool:
        ...
        try:
            if self.risk_manager:
                return await self.risk_manager.reset_weekly_counters()
            else:
                return False
        except Exception as e:
