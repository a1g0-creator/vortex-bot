...
class ConfigLoader:
    ...
    def get_exchange_config(self, exchange: str) -> Dict[str, Any]:
        """
        Получение конфигурации биржи для обратной совместимости
        Алиас для get_active_exchange_config
    
        Args:
            exchange: Название биржи
        
        Returns:
            Конфигурация активного окружения биржи
        """
        return self.get_active_exchange_config(exchange)

    def get_exchange_credentials(self, exchange: str, environment: str = None) -> Dict[str, str]:
        """
        Получение API credentials для биржи
    
        Args:
            exchange: Название биржи
            environment: Окружение (demo/mainnet). Если None - используется активное
        
        Returns:
            Словарь с API ключами
        """
        if environment is None:
            environment = self.get_active_environment(exchange)
    
        exchange_config = self.configs.get("exchanges", {}).get(exchange, {})
        api_credentials = exchange_config.get("api_credentials", {})
    
        # Поддержка разных названий окружений
        env_aliases = {
            "demo": ["demo", "testnet"],
            "mainnet": ["mainnet", "prod", "production"]
        }
    
        # Ищем credentials в разных вариантах
        for env_key in env_aliases.get(environment, [environment]):
            if env_key in api_credentials:
                return api_credentials[env_key]
    
        return {}

    def get_active_exchange_env(self, name: str) -> str:
        """
        ПАТЧ: Добавлен метод для получения активного окружения биржи
        Алиас для совместимости со смоук-тестом
    
        Args:
            name: Название биржи (bybit, bitget)
        
        Returns:
            Название активного окружения ("demo", "mainnet" и т.д.)
        """
        # Используем существующий метод get_active_environment
        env = self.get_active_environment(name)
        # Возвращаем окружение или "demo" по умолчанию
        return env if env else "demo"
