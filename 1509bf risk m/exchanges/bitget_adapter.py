...
class BitgetAdapter(BaseExchange):
    ...
    def _pt(self) -> str:
        """Корректный productType с учётом demo/mainnet."""
        return "susdt-futures" if self.testnet else "USDT-FUTURES"

    async def _ensure_position_mode(self, target: str = "one_way") -> bool:
        """
        Гарантирует нужный режим позиций на аккаунте.
        Поддерживает оба варианта Bitget V2:
        - positionMode: one_way | hedge
        - holdMode: single_hold | double_hold (альтернативный параметр)
        """
        try:
            ...
            # 1) Современный вариант с holdMode
            hold_mode_map = {"one_way": "single_hold", "hedge": "double_hold"}
            payload1 = {
                "productType": product_type,
                "marginCoin": "USDT",
                "holdMode": hold_mode_map.get(target, "single_hold")
            }
            ...
            # 2) Fallback на positionMode
            payload2 = {
                "productType": product_type,
                "marginCoin": "USDT",
                "positionMode": target  # "one_way" | "hedge"
            }
            ...
            # 3) Legacy вариант с posMode
            legacy_map = {"one_way": "one_way_mode", "hedge": "long_short_mode"}
            payload3 = {
                "productType": product_type,
                "marginCoin": "USDT",
                "posMode": legacy_map.get(target, target)
            }
            res3 = await self._make_request(
                "POST",
                "/api/v2/mix/account/set-position-mode",
                data=payload3,
                signed=True
            ...
            self.logger.warning("Failed to set position mode; proceeding without cache")
            return False
    ...
    def _get_headers(self, method: str, endpoint: str, params: dict = None) -> Dict[str, str]:
        """
        Получение заголовков для HTTP запроса (сигнатура BaseExchange)
        ...
        # В demo-режиме добавляем paptrading=1 для приватных эндпоинтов
    ...
    async def _make_request(
        ...
        try:
            ...
            # Добавляем авторизацию для приватных запросов
            if signed:
                auth_headers = self._generate_auth_headers(method, endpoint, query_string, body)
            ...
            async with self.session.request(
                ...
                # Специальная обработка ошибок для market reduceOnly без позиции
                if response.status == 400 and ("22002" in text or "No position to close" in text):
                    self.logger.info(f"No position to close - treating as successful no-op")
                    return {"noop": True, "reason": "no_position"}
                
                if response.status != 200:
                    self.logger.error(f"HTTP {response.status} for {endpoint}: {text}")
                    return None
                    
                try:
                    payload = json.loads(text)
                except json.JSONDecodeError:
                    self.logger.error(f"JSON parse error: {text}")
                    return None

                # Bitget успешный код: "00000"
                if str(payload.get("code")) in ("00000", "0"):
                    return payload.get("data")
                else:
                    self.logger.error(f"Bitget API error: {payload}")
                    return None
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Устанавливаем one-way режим
            await self._ensure_position_mode("one_way")
            
            self.logger.info(f"Initialized successfully, server time: {server_time}")
            return True
    ...
    async def get_balance(self, coin: str = "USDT") -> Optional[Balance]:
        ...
        try:
            ...
            if response:
                return Balance(
                    coin=coin,
                    wallet_balance=float(response.get("equity", 0)),
                    available_balance=float(response.get("available", 0)),
                    used_balance=float(response.get("frozen", 0)),
                    unrealized_pnl=float(response.get("unrealizedPL", 0))
                )

            # запасной вариант — список аккаунтов
            response = await self._make_request(
                "GET",
            ...
            if response:
                accounts = response if isinstance(response, list) else [response]
                for account in accounts:
                    if account.get("marginCoin") == coin:
                        return Balance(
                            coin=coin,
                            wallet_balance=float(account.get("equity", 0)),
                            available_balance=float(account.get("available", 0)),
                            used_balance=float(account.get("frozen", 0)),
                            unrealized_pnl=float(account.get("unrealizedPL", 0))
                        )

            return Balance(coin=coin, wallet_balance=0.0, available_balance=0.0, used_balance=0.0, unrealized_pnl=0.0)
    ...
    async def get_wallet_balance(self) -> List[Balance]:
        """Получить все балансы кошелька (для demo — фактически USDT)."""
        try:
    ...
    async def get_server_time(self) -> int:
        ...
        try:
            ...
            if response:
                return int(response.get("serverTime", 0))
