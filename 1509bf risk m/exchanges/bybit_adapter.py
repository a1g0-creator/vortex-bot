...
class BybitAdapter(BaseExchange):
    ...
    def __init__(
        ...
        *,
        ...
        """
        Initialize Bybit v5 adapter - supports multiple initialization patterns
        
        Args:
            api_key: API key
            api_secret: API secret
            base_url: REST API base URL (optional, uses default if not provided)
            ws_public: Public WebSocket URL (optional, uses default if not provided)
            ws_private: Private WebSocket URL (optional, uses default if not provided)
            testnet: Use demo environment (for backward compatibility)
            recv_window: Request receive window in ms
            config: Configuration dictionary (alternative init method)
        """
        ...
        if config is not None:
            ...
            # Initialize base class (testnet flag for backward compatibility)
            testnet = config.get("environment") == "demo"
            super().__init__(api_key, api_secret, testnet)
            
            # Set environment configuration from config
            self.environment = config.get("environment", "demo")
            ...
            # Environment (demo vs mainnet)
            self.environment = "demo" if testnet else "mainnet"
            
            # REST/WS endpoints
            self._base_url = base_url or "https://api-demo.bybit.com"
            self._ws_public_url = ws_public or "wss://stream-demo.bybit.com/v5/public/linear"
            self._ws_private_url = ws_private or "wss://stream-demo.bybit.com/v5/private"
        ...
        # Log initialization
        self.logger.info(f"BybitAdapter initialized:")
